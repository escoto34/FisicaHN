...
-- ============ WAVE 6 §6.4 ============
-- Migración 0001 — esquema histórico CONGELADO (migración cero, §6.2).
-- NOTA: se registra al final, siempre idempotente.
...
begin
  if not exists (select 1 from public.schema_migrations where version = 1) then
    ...
    values (1, 'ANALIZED_100%');
  else
    raise notice '0001 ya aplicada — esquema en orden';
  end if;
end $$;

-- REVERSIÓN: no aplicable (0001 = estado inicial; se revierte restaurando un backup).
