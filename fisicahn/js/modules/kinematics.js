/**
 * @fileoverview Facade / Enrutador del módulo de Cinemática.
 * Permite seleccionar y ejecutar: MRU, MRUV, Caída Libre, Tiro Parabólico.
 */

import * as mru from './kinematics/mru.js';
import * as mruv from './kinematics/mruv.js';
import * as freeFall from './kinematics/free-fall.js';
import * as projectile from './kinematics/projectile.js';

const SUB_MODULES = {
  mru: { name: 'Movimiento Rectilíneo Uniforme (MRU)', module: mru },
  mruv: { name: 'Movimiento Rectilíneo Uniformemente Variado (MRUV)', module: mruv },
  'free-fall': { name: 'Caída Libre', module: freeFall },
  projectile: { name: 'Tiro Parabólico', module: projectile }
};
...
export function update(dt) {
  if (activeSub && typeof activeSub.update === 'function') {
    // Pasar elapsed time desde el engine
...
function renderSelectorHeader() {
  ...
  selectorDiv.innerHTML = `
    <label class="control-label" style="font-weight:bold;color:var(--accent)">Sub-módulo de Cinemática</label>
    <select class="select-input" id="kin_submodule_select" style="width:100%;margin-top:6px;padding:6px;background:var(--bg-tertiary);color:var(--text-primary);border:1px solid var(--border-color);border-radius:var(--radius-sm)">
      ${optionsHTML}
    </select>
  `;
  ...
  // Vincular evento
  const select = document.getElementById('kin_submodule_select');
