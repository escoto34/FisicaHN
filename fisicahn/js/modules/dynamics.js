/**
 * @fileoverview Facade / Enrutador del módulo de Dinámica.
 * Permite seleccionar y ejecutar: Leyes de Newton, Trabajo y Energía.
 */

import * as newtonLaws from './dynamics/newton-laws.js';
import * as workEnergy from './dynamics/work-energy.js';

const SUB_MODULES = {
  newton: { name: 'Leyes de Newton con Fricción', module: newtonLaws },
  'work-energy': { name: 'Trabajo, Energía y Potencia', module: workEnergy }
};

let activeKey = 'newton';
let activeSub = newtonLaws;
let _engine = null;
let _renderer = null;
let _ui = null;
...
function renderSelectorHeader() {
  ...
  selectorDiv.innerHTML = `
    <label class="control-label" style="font-weight:bold;color:var(--accent-secondary)">Sub-módulo de Dinámica</label>
    <select class="select-input" id="dyn_submodule_select" style="width:100%;margin-top:6px;padding:6px;background:var(--bg-tertiary);color:var(--text-primary);border:1px solid var(--border-color);border-radius:var(--radius-sm)">
      ${optionsHTML}
    </select>
  `;
  ...
  // Vincular evento
  const select = document.getElementById('dyn_submodule_select');
